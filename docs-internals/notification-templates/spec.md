...
| GET | `/notification-templates/{channel}/types/{typeId}/resolve?ouId=&appId=&locale=` | OU | Resolve the branding+locale-applied notification (also drives the Console preview; `{{ctx}}` placeholders shown as-is) |
...
Method choice: `resolve` is GET (scalar inputs `ouId`, `appId`, `locale`), matching `design/resolve`.
The Console preview renders the same `resolve` output — no separate sample-data step; `{{ctx(...)}}`
placeholders are shown as-is.

### UI

Notification templates live under the Console's **Customization** section (alongside Design &
Branding), and are surfaced at **two scopes** that mirror the design: the **root organization** (manage
the type catalog + author base content) and the **organization unit** (override inherited content). The
mockups follow the Console's dark theme and page style. Mockups source: `assets/*.html`, rendered to PNG.

#### Root organization — types + base content

![Root-organization notification templates](assets/templates-root.png)

At the root scope an administrator manages the **global type catalog** — `+ Custom template` creates a
custom type, and custom types can be renamed or deleted (system types are read-only) — and authors the
**base content** per type and locale that every OU inherits. The **Base content** column shows *System
default* vs *Edited at root*; **Reset** returns a system type's base to its shipped default.

#### Organization unit — content overrides

![Organization-unit notification templates](assets/templates-ou.png)

At an OU scope the type catalog is **read-only** (managed at root — no `+ Custom template`). The
**Content source** column shows where each template's effective content resolves from — *inherited
(system)*, *inherited (root org / ancestor)*, or *customized · this OU*. **Edit** creates a copy-on-write
override for this OU and its descendants; **Revert** (shown only where an override exists) drops it and
falls back to the inherited or system value.

#### Template editor with live branded preview

Used at both scopes. ![Email template editor with branded preview](assets/email-editor.png)

The left pane edits the **content** — subject and body with `{{ctx(...)}}` placeholders. The right pane
is a **live preview** of the notification exactly as delivered: resolved content + composed branding
(logo, colors, header/footer) + selected locale, with `{{ctx(...)}}` shown **as-is** (values are
substituted only at send time). Prominent scope indicators show the two resolution chains
(**Content** = Engineering OU, **Branding** = Acme). **Revert** offers *to parent OU (or root org)* or
*to default (shipped)*.
...
**Requirement:** Resolve returns the effective, branding- and locale-applied notification; the Console
renders that same output as a preview, with `{{ctx(...)}}` placeholders shown as-is.
...
- **AC9.2:** Given a resolve for an OU, when the Console previews it, then the branded, localized
  notification is shown with `{{ctx(...)}}` placeholders displayed as-is (not substituted).
