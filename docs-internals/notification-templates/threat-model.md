...
| Authentication | OAuth2 `system` scope | |
| Accessibility | [Restricted] | Admin only. |
| Authorization and Access Control | `ouId` ownership check identical to write path | Preview uses admin-supplied sample data, not real user PII. |
...
| 3 | [Privacy Risk] | Real user PII appears in preview. | [No] | Preview substitutes admin-provided sample data; it does not fetch real recipient attributes. |
...
| 5 | Access to personal data on need-to-know? | [Yes] | Preview uses sample data; runtime data is scoped to the flow. |
