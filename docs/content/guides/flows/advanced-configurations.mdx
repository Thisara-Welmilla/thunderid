...
Credential-bearing headers (`Authorization`, `Cookie`, `Set-Cookie`, `Proxy-Authorization`) are stripped and are never available through this syntax. If a request, header, or query parameter is not present, the placeholder is kept as-is.
